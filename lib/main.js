...
var uaPanel = panels.Panel({
  	...
  	contentScriptFile: [data.url('js/jquery-1.8.0.min.js'),
	...
	onMessage: function(data) {
		if(data != 'focus'){
			if(data != null && data != ''){
					if(data.match(/^default/gi) || data.match(/^firefox/gi)){
						// ( if message == default ) then reset the UA
						prefs.reset("general.useragent.override");
						data = "Default browser userAgent";
						...
						data = iOSUA;
					}else if (data.match(/^chrome/gi)){
						// ( if message match chrome) then set UA to chrome useragent
						// iOS user Agent "chrome 22.0"
						var chromeUA = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/22.0.1207.1 Safari/537.1";
						prefs.set("general.useragent.override",chromeUA);
						// setting data to chrome UA to appear in notifications
						data = chromeUA;
					}else if (data.match(/^safari/gi)){
						// ( if message match safari) then set UA to safari useragent
						// iOS user Agent "safari 6.0"
						var safariUA = "Mozilla/5.0 (iPad; CPU OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A5355d Safari/8536.25";
						prefs.set("general.useragent.override",safariUA);
						// setting data to safari UA to appear in notifications
						data = safariUA;
					}else if (data.match(/^ie/gi)){
						// ( if message match IE) then set UA to IE useragent
						// iOS user Agent "IE 10.6"
						var ieUA = "Mozilla/5.0 (compatible; MSIE 10.6; Windows NT 6.1; Trident/5.0; InfoPath.2; SLCC1; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET CLR 2.0.50727) 3gpp-gba UNTRUSTED/1.0";
						prefs.set("general.useragent.override",ieUA);
						// setting data to IE UA to appear in notifications
						data = ieUA;
...
var internetExplorer = cm.Menu({
  ...
  items: [
	...
	cm.Item({ label: "IE10.6", data: "Mozilla/5.0 (compatible; MSIE 10.6; Windows NT 6.1; Trident/5.0; InfoPath.2; SLCC1; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET CLR 2.0.50727) 3gpp-gba UNTRUSTED/1.0" }),
]
});

// Google Chrome 15 UA
var googleChrome = cm.Item({ label: "Google Chrome", data: "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/22.0.1207.1 Safari/537.1" });

// safari 5.0.5 UA
var safari = cm.Item({ label: "Safari 6", data: "Mozilla/5.0 (iPad; CPU OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A5355d Safari/8536.25" });
